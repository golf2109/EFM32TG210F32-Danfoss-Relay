#include "danfoss.h"
extern  void  send_danfoss(uint8_t data);
extern  uint8_t counter_from_danfoss;
extern  void  send_danfoss(uint8_t data);
	
void  read_danfoss(void)
{
	counter_from_danfoss = 0;
}

void  start_danfoss(void)
{
//static  uint8_t set_frq_danfoss[6] = {0x01, 0x06, 0xc3, 0x4f, 0x04, 0x7c,};	
//static  uint16_t crc_danfoss;
//static  uint8_t  crc_1, crc_2;
	
send_danfoss(0x01);		
send_danfoss(0x06);
send_danfoss(0xc3);
send_danfoss(0x4f);
send_danfoss(0x04);
send_danfoss(0x7c);
send_danfoss(0x87);
send_danfoss(0x78);		

counter_from_danfoss = 0;	
read_danfoss();
}

void  stop_danfoss(void)
{	
send_danfoss(0x01);		
send_danfoss(0x06);
send_danfoss(0xc3);
send_danfoss(0x4f);
send_danfoss(0x04);
send_danfoss(0x2c);
send_danfoss(0x87);
send_danfoss(0x44);		

counter_from_danfoss = 0;		
read_danfoss();	
}

void  set_frq_danfoss(uint8_t frq_danfoss_h, uint8_t frq_danfoss_l)
{
static  uint8_t set_frq_danfoss[6] = {0x01, 0x06, 0xc3, 0x59, 0x00, 0x00,};	
static  uint16_t crc_danfoss;
start_danfoss();
Delay(100);
set_frq_danfoss[4] = (uint8_t)((32*(256*frq_danfoss_h+frq_danfoss_l))>>8);
set_frq_danfoss[5] = (uint8_t)(32*(256*frq_danfoss_h+frq_danfoss_l));

crc_danfoss = crc16(set_frq_danfoss, 6);	

send_danfoss(0x01);		
send_danfoss(0x06);
send_danfoss(0xc3);
send_danfoss(0x59);
//send_danfoss(0x10);
//send_danfoss(frq_danfoss_h);	
send_danfoss(set_frq_danfoss[4]);	
//send_danfoss(0x00);
//send_danfoss(frq_danfoss_l);	
send_danfoss(set_frq_danfoss[5]);	
//crc_1 = (uint8_t)(crc_danfoss>>8);
//crc_2 = (uint8_t)(crc_danfoss);
//send_danfoss(0x68);
send_danfoss((uint8_t)(crc_danfoss>>8));
//send_danfoss(0x5d);	
send_danfoss((uint8_t)crc_danfoss);

counter_from_danfoss = 0;	
read_danfoss();
}	


void  test_danfoss(void)
{
start_danfoss();	
Delay(100);		
//set_frq_danfoss(0x10, 0x00);//12.8 Hz
set_frq_danfoss(0x00, 0x80);//12.8 Hz	
Delay(10000);	
//set_frq_danfoss(0x20, 0x00);//25.6 Hz
set_frq_danfoss(0x01, 0x00);//25.6 Hz	
Delay(10000);	
//set_frq_danfoss(0x30, 0x00);//38.4 Hz
set_frq_danfoss(0x01, 0x80);//38.4 Hz	
Delay(10000);	
//set_frq_danfoss(0x40, 0x00);//51.2 Hz
set_frq_danfoss(0x02, 0x00);//51.2 Hz	
//set_frq_danfoss(0x3e, 0x80);//50.0 Hz	
//set_frq_danfoss(0x01, 0xf4);//50.0 Hz		
Delay(10000);	
stop_danfoss();
Delay(5000);
}

uint16_t crc16(uint8_t *adr_buffer, uint32_t byte_cnt)
{
    static unsigned char auchCRCHi[]=
    {
        ,
    };
    /*Table of CRC values for low–order byte*/
    static char auchCRCLo[]=
    {
        ,
    };

    unsigned char uchCRCHi = 0xFF;  
    unsigned char uchCRCLo = 0xFF; 
    unsigned uIndex; 
    
    /* CRC Generation Function */
    while( byte_cnt--) /* pass through message buffer */
    {
        uIndex = uchCRCHi ^ *adr_buffer++; /* calculate the CRC */
        uchCRCHi = uchCRCLo ^ auchCRCHi[uIndex];
        uchCRCLo = auchCRCLo[uIndex];
    }
return (uchCRCHi << 8 | uchCRCLo);
}

